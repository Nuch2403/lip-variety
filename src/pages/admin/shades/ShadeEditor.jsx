...
export default function ShadeEditor() {
  ...
  const [promptMessage] = useState("มีการแก้ไขที่ยังไม่ได้บันทึก ต้องการออกจากหน้านี้หรือไม่?");
  ...
  const isDirty = useMemo(() => {
    ...
    const norm = (obj) => ({
      ...
      shade_name: (obj.shade_name || "").trim(),
      shade_code: obj.shade_code || "",
      hex: obj.hex || "",
      undertone_tag: obj.undertone_tag || "",
      depth_tag: obj.depth_tag || "",
    ...
    (async () => {
      ...
      if (data) {
        const normalized = {
          ...
          hex: data.hex ?? "",
          undertone_tag: data.undertone_tag ?? "",
          depth_tag: data.depth_tag ?? "",
  ...
  async function save(e) {
    ...
    const payload = {
      ...
      shade_name: form.shade_name.trim(),
      shade_code: form.shade_code.trim() || null,
      hex: form.hex.trim() || null,
      undertone_tag: form.undertone_tag.trim() || null,
      depth_tag: form.depth_tag.trim() || null,
    ...
    try {
      if (isNew) {
        ...
        const norm = { ...payload, product_id: payload.product_id || "" };
        setInitialForm(norm);
        setForm(norm);
  ...
  function del() {
    ...
    setConfirmState({
      open: true,
      message: "ยืนยันลบเฉดสีนี้หรือไม่?",
      onConfirm: async () => {
        ...
        <div className="space-y-3 rounded-2xl border p-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                onChange={(e) => setField("shade_name", e.target.value)}
                required
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) => setField("shade_code", e.target.value)}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            <div>
              ...
              <select
                className="mt-1 w-full rounded-xl border px-3 py-2"
                value={form.undertone_tag}
                onChange={(e) => setField("undertone_tag", e.target.value)}
              >
            ...
            <div>
              ...
              <select
                className="mt-1 w-full rounded-xl border px-3 py-2"
                value={form.depth_tag}
                onChange={(e) => setField("depth_tag", e.target.value)}
              >
          ...
          <div className="flex gap-2">
            ...
            {!isNew ? (
              <button type="button" className="rounded-xl border px-4 py-2 text-sm" onClick={del}>
                Delete
              </button>
            ) : null}
          </div>
