    ...
    <div className="p-5 md:p-6">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <div className="text-sm text-zinc-600">จัดการเฉดสีของสินค้า ระบุโทนสี และรูปตัวอย่างสี</div>
        </div>
        <Link to="/admin/shades/new" className="rounded-xl bg-zinc-900 text-white px-4 py-2 text-sm hover:opacity-90">+ Create</Link>
      </div>

      <input className="mt-4 w-full rounded-xl border px-3 py-2" value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search shade / product / brand / hex…" />
            ...
            {filtered.map((r) => (
              <tr key={r.id} className="border-b">
                <td className="py-2 pr-3">
                  ...
                  {r.shade_code ? <div className="text-xs text-zinc-500 font-mono">{r.shade_code}</div> : null}
                </td>
                <td className="py-2 pr-3">
                ...
                <td className="py-2 pr-3 font-medium">{r._product?.model ?? ""}</td>
                ...
                <td className="py-2 pr-3">
                  <div className="flex items-center gap-2">
                    <Link className="text-xs underline" to={`/admin/shades/${r.id}`}>Edit</Link>
                    {r._product ? <Link className="text-xs underline text-zinc-500" to={`/admin/products/${r._product.id}`}>Product →</Link> : null}
                  </div>
                </td>
              </tr>
            ))}
            {!filtered.length ? <tr><td colSpan={8} className="py-8 text-center text-zinc-500">No shades found</td></tr> : null}
          </tbody>
        </table>
      </div>
    </div>
