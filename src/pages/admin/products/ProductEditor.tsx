...
interface ProductForm {
  model: string; brand_id: string | number; type_tag: string;
  finish_id: string | number; longevity_tag: string;
  cover_image_path: string; is_discontinued: boolean;
}
...
const blankForm: ProductForm = {
  model: "", brand_id: "", type_tag: "", finish_id: "",
  longevity_tag: "", cover_image_path: "", is_discontinued: false,
};
...
export default function ProductEditor() {
  ...
  const [lipTypes, setLipTypes] = useState<{ code: string; name_th?: string; name_en?: string }[]>([]);
  const [finishes, setFinishes] = useState<{ id: number; name: string; slug: string }[]>([]);
  const [toast, setToast]       = useState("");
  const [form, setForm]         = useState<ProductForm>(blankForm);
  const [initialForm, setInitialForm] = useState<ProductForm | null>(null);
  ...
  const isDirty = useMemo(() => {
    ...
    const norm = (obj: ProductForm) => ({
      model: (obj.model || "").trim(), brand_id: obj.brand_id || "",
      type_tag: obj.type_tag || "", finish_id: obj.finish_id || "",
      longevity_tag: obj.longevity_tag || "", cover_image_path: obj.cover_image_path || "",
      is_discontinued: !!obj.is_discontinued,
    });
    return JSON.stringify(norm(initialForm)) !== JSON.stringify(norm(form));
  }, [form, initialForm]);

  const setField = <K extends keyof ProductForm>(k: K, v: ProductForm[K]) => setForm((s) => ({ ...s, [k]: v }));

  useEffect(() => {
    (async () => {
      const [{ data: brandData }, { data: typeData }, { data: finishData }] = await Promise.all([
        supabase.from("brands").select("id,name").order("name", { ascending: true }),
        supabase.from("lip_types").select("code,name_th,name_en,sort_order").order("sort_order", { ascending: true }).order("name_th", { ascending: true }),
        supabase.from("finishes").select("id,name,slug,is_active").eq("is_active", true).order("name", { ascending: true }),
      ]);
      setBrands((brandData as typeof brands) ?? []);
      setLipTypes((typeData as typeof lipTypes) ?? []);
      setFinishes((finishData as typeof finishes) ?? []);
    })();
  }, []);
    ...
    (async () => {
      ...
      if (data) {
        const normalized: ProductForm = {
          model: data.model ?? "", brand_id: data.brand_id ?? "",
          type_tag: data.type_tag ?? "", finish_id: data.finish_id ?? "",
          longevity_tag: data.longevity_tag ?? "", cover_image_path: data.cover_image_path ?? "",
          is_discontinued: !!data.is_discontinued,
        };
        let mappedFinishId = normalized.finish_id || "";
        if (!mappedFinishId && data.finish_tag && finishes.length) {
          const match = finishes.find((f) => (f.slug || "").trim().toLowerCase() === (data.finish_tag || "").trim().toLowerCase());
          if (match) mappedFinishId = match.id;
        }
        const nextForm = { ...normalized, finish_id: mappedFinishId };
  ...
  async function save(e: React.FormEvent) {
    ...
    const payload = {
      model: form.model.trim(), brand_id: toInt(form.brand_id),
      type_tag: form.type_tag || null, finish_id: form.finish_id || null,
      longevity_tag: String(form.longevity_tag).trim() || null,
      cover_image_path: form.cover_image_path || null, is_discontinued: !!form.is_discontinued,
    };
    try {
      if (isNew) {
        ...
        const norm = { ...payload, brand_id: payload.brand_id ?? "", finish_id: payload.finish_id ?? "", cover_image_path: payload.cover_image_path ?? "" } as ProductForm;
  ...
  function confirmDelete() {
    if (isNew) return;
    setConfirmState({
      open: true, message: "ยืนยันลบสินค้านี้หรือไม่?",
      onConfirm: async () => {
        const { error } = await supabase.from("products").delete().eq("id", id);
        if (error) { setToast(error.message ?? "ลบไม่สำเร็จ"); }
        else { setToast("ลบแล้ว"); nav("/admin/products", { replace: true }); }
  ...
  return (
    <div className="p-5 md:p-6 space-y-5">
      ...
      <form className="grid grid-cols-1 gap-4 lg:grid-cols-[1fr_420px]" onSubmit={save}>
        ...
        <div className="space-y-3">
          <ImageUploader label="Product image" folder={`products/${form.model || id || "product"}`} value={form.cover_image_path} onChangePath={(p) => setField("cover_image_path", p)} />
        </div>
      </form>
