...
/**
 * หน้า "รุ่นลิปสติกน่าใช้ — คัดให้แล้ว"
 * - read-only จาก Supabase
 * - ไม่พึ่ง /admin (เพราะถอดระบบ Admin ออกแล้ว)
 * - ใช้ cover_image_path จาก products เป็นรูปเดียว
 */
export default function RecommendModels() {
  ...
  async function fetchData() {
    ...
    try {
      let q = supabase
        ...
        .select(
          `
          id, model, type_tag, finish_tag, longevity_tag, is_discontinued, cover_image_path,
          finish_id, finishes ( id, name, slug ),
      ...
      const normalized =
        (data || []).map((p) => {
          return {
            ...p,
            _img: resolveImage(p.cover_image_path),
            _shadeCount: (p.shades || []).length,
          };
        }) || [];
  ...
  const options = useMemo(() => {
    ...
    return {
      ...
      longevity: Array.from(longs).sort(),
    };
  }, [list, finishOptions]);
  ...
  return (
    <div className="container-page py-10">
      ...
      <p className="text-zinc-600 mb-6">
        เลือกดูรุ่นลิปจากข้อมูลในฐาน (read-only) และคัดลอกชื่อไปค้นหาได้ทันที
      </p>
      ...
      {!loading && err && (
        <div className="rounded-xl border bg-amber-50 p-4 text-amber-900">{err}</div>
      )}
              ...
              <div key={p.id} className="rounded-2xl border bg-white shadow-sm overflow-hidden">
                ...
                <div className="p-4">
                  ...
                  <div className="text-sm text-zinc-600">
                    {brand || "—"} • {String(p.type_tag)}{" "}
                    {p.finishes?.name ? `• ${p.finishes.name}` : p.finish_tag ? `• ${p.finish_tag}` : ""}{" "}
                    {p.longevity_tag ? `• ${String(p.longevity_tag)}` : ""}
                  </div>

                  <div className="mt-2 text-xs text-zinc-500">
                    จำนวนเฉด: {p._shadeCount}
                  </div>
