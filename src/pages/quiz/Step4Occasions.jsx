...
export default function Step4Occasions(){
  ...
  useEffect(()=>{
    ...
    else if (Array.isArray(q.occasions) && q.occasions.length) setVal(q.occasions[0]);
  },[]);

  const next = ()=>{
    if (!val) return;
    patchQuiz({ occasions: val }); // เก็บเป็น string เดี่ยวให้ตรงกับ Results
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {OPTIONS.map(o=>(
          <label key={o.code}
            ...
            <input
              type="radio" name="occasions" className="sr-only"
              checked={val===o.code} onChange={()=>setVal(o.code)} aria-label={o.label}
            />
